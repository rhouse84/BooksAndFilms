...
public class UserListPresenter implements Presenter {
	...
	public void bind() {
		...
		eventBus.addHandler(UserUpdatedEvent.TYPE, new UserUpdatedEventHandler() {
	    	@Override public void onUpdateUser(UserUpdatedEvent event) {
	        	fetchUserSummary();
	    	}
	    });

		eventBus.addHandler(UserDeleteEvent.TYPE, new UserDeleteEventHandler() {
			@Override public void onDeleteUser(UserDeleteEvent event) {
				fetchUserSummary();
			}
		});
	...
	private void fetchUserSummary() {
		new RPCCall<ArrayList<UserAccount>>() {
			@Override protected void callService(AsyncCallback<ArrayList<UserAccount>> cb) {
				rpcService.getAllUsers(cb);
			}
			
			@Override public void onSuccess(ArrayList<UserAccount> result) {
				users = result;
